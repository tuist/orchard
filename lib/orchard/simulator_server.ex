defmodule Orchard.SimulatorServer do
  ...
  @check_interval 1_000  # Check simulator state every second
    ...
    case get_simulator_state(state.udid) do
      {:ok, current_state} ->
        ...
        if current_state != state.state do
          Logger.info("Simulator #{state.name} (#{state.udid}) state changed: #{state.state} -> #{current_state}")
      ...
      {:error, :not_found} ->
        ...
        Logger.info("Simulator #{state.name} (#{state.udid}) no longer exists, terminating server")
        {:stop, :normal, state}
  ...
  defp perform_tap(udid, x, y) do
    case MuonTrap.cmd(Config.axe_cmd(), ["tap", "--udid", udid, "--x", to_string(x), "--y", to_string(y)]) do
      {_, 0} -> :ok
      {error, _} -> {:error, error}
    end
  end
  ...
  defp get_simulator_state(udid) do
    case MuonTrap.cmd(Config.axe_cmd(), ["list-simulators"]) do
      {output, 0} ->
        ...
        simulator_line = Enum.find(lines, fn line ->
